...
```{r}
ggplot(data = wildfire_counts, aes(x = ACQ_DATE, y = n_fire)) +
  geom_line()
```
...
```{r}
ggplot(data = wildfire_shp_data, aes(y = ACQ_DATE, fill = n)) +
  geom_col()
```
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1753224078180.kml",
...
```{r}
ch3_df <- st_read(here("data/LANDSAT_USA_contiguous_and_Hawaii_7d.csv"))

#ch3_sf <- st_as_sf(ch3_df, wkt = "latitude", "longitude") 
                            #wkt doesn't work bc lat+long arent in one column?

sf_df <- st_as_sf(ch3_df, coords = c("longitude", "latitude"), crs = 4326)#3857 for google maps
```

```{r}
mapboxgl(bounds = sf_df) |>
  add_circle_layer(
    id = "wildfires_categorical",
    source = sf_df,
    circle_radius = 4,
