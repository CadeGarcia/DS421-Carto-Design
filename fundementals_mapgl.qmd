...
continuous_scale <- interpolate_palette(
  ...
  method = "equal",
  n = 5,
  # I think this color works better for this data because the contrast is more harsh. It shows more contrast in the data
  palette = viridis
)
...
**There tends to be more green when I change the code. It seems that the median value goes shifted and the map adjusts with the data. I think step_equal_interval is the most balanced. If the story is trying to draw contrast, I think step_quantile would still be the best option.**
...
hi_median_income <- hi_median_income |>
  mutate(
    mit_threshold = ifelse(estimate < 107689, "Below the Line", "Above the Line")
  )
```

```{r}

mit_colors <- c("Below the Line" = "#d73027", "Above the Line" = "#1a9850")
...
hi_house_built <- get_acs(
  geography = "tract",
  variables = "B28002_002",
  state = "HI",
  year = 2020,
  geometry = TRUE
)

hi_scale <- interpolate_palette(
  data = hi_house_built,
  column = "estimate",
  method = "equal",
  n = 4,
...
mapboxgl(bounds = hi_house_built) |>
  add_fill_layer(
    id = "hi_tracts",
    source = hi_house_built,
    fill_color = hi_scale$expression,
    fill_opacity = 0.7,
    popup = "popup",
    tooltip = "estimate",
    hover_options = list(
  ...
  add_legend(
    "Households with an internet Subscription in HI",
    values = get_legend_labels(hi_scale, digits = 0),
    colors = get_legend_colors(hi_scale),
    type = "categorical"
  )
# confused about the tract. My labels may be wrong!
```
